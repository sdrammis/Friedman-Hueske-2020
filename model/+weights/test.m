...
scoreWTOld = findbest(weights.WTOld, ENG, ITTR);
scoreWTNL = findbest(weights.WTNL, ENG, ITTR);
scoreHDNL = findbest(weights.HDNL, ENG, ITTR);

t1Arr = nan(1,ITTR);
t2Arr = nan(1,ITTR);
for i=1:ITTR
    [t1Arr(i), t2Arr(i)] = runnet(weights.WTYng,ENG);
end
t1 = mean(t1Arr);
t2 = mean(t2Arr);
F= build_score_fn();
