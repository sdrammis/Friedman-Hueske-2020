...
CTBase(nanIdxs) = [];
mouseIDs(nanIdxs) = [];
figure() % Plot reward and cost trace
subplot(1,2,1)
for i = 1:length(RTWater)
    hold on
    plotNoBar_UPDATE2({RTBase{i},RTWater{i}},'Reward Trace',{'Base','Deval'},...
        strioStr,'b','b','b',0,0)
    hold on
    plotNoBar_UPDATE2({RTBase{i},RTSucrose{i}},'Reward Trace',{'Base','Deval'},...
        strioStr,'r','r','r',0,0)
    hold off
    hold off
end
...
for i = 1:length(CTWater)
    hold on
    plotNoBar_UPDATE2({CTBase{i},CTWater{i}},'Cost Trace',{'Base','Deval'},...
        strioStr,'b','b','b',0,0)
    hold on
    plotNoBar_UPDATE2({CTBase{i},CTSucrose{i}},'Cost Trace',{'Base','Deval'},...
        strioStr,'r','r','r',0,0)
    hold off
    hold off
end
