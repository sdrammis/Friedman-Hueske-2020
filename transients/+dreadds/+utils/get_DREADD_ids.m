function miceIDs = get_DREADD_ids(twdb,taskReversal,DREADDType,health,learned,...
    intendedStriosomality,histologyStriosomality,age,photometry,exceptions)
 
mice_idx = 1:length(twdb);
 
if ~isequal(DREADDType,'all')
    mice_idx = cell2mat(twdb_lookup(twdb(mice_idx),'index','key','DREADDType',DREADDType));
end
...
miceIDs = unique(twdb_lookup(twdb(mice_idx),'mouseID','key','injection','CNO'));
