function dreaddBehaviorStatsGivenSpikes_inhibitory(twdb,dreaddGroups,intendedStriosomality,health,learned,DREADDType)
...
reversal = 0;
miceIDs = get_DREADD_ids(twdb,reversal,DREADDType,health,learned,...
        intendedStriosomality,'all','all','all',{});
...
for m = 1:length(miceIDs)
    ...
    for s = 1:length(cnoSessions)
        if ~dreaddGroups.hasTransients(cnoSessions(s))
            continue
        end
...
supertitle({[title_str ' n = ' num2str(length(unique(used_mice)))],''})
