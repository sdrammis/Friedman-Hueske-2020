function overallCorrelation(twdb,bin_size,dp_size,intendedStriosomality,health,learned,age,engagement,xType,yType,learningPeriod)
    ...
    for m = 1:length(miceIDs)
        ...
        meanLicks = nanmean(mouseTrials.ResponseLickFrequency);
        stdLicks = nanstd(mouseTrials.ResponseLickFrequency);
        ...
        for b = 1:length(bins)-1
            ...
            traceSum = get_dist_stats(bin_fluorTrials);
            responseTraceSum = sum(traceSum(responsePeriod));
            
            [rewardTrials,costTrials] = reward_and_cost_trials(bin_mouseTrials,bin_fluorTrials,rewardTone,costTone);
            rewardLicks = (rewardTrials.ResponseLickFrequency - meanLicks)/stdLicks;
            costLicks = (costTrials.ResponseLickFrequency - meanLicks)/stdLicks;
            licks = (bin_mouseTrials.ResponseLickFrequency - meanLicks)/stdLicks;
            ...
            mouse_sum = [mouse_sum responseTraceSum];
            mouse_rewardLicks = [mouse_rewardLicks nanmean(rewardLicks)];
            mouse_costLicks = [mouse_costLicks nanmean(costLicks)];
            mouse_licks = [mouse_licks nanmean(licks)];
        ...
        elseif isequal(xType,'Reward Licks')
            x_overall = [x_overall mouse_rewardLicks];
        elseif isequal(xType,'Cost Licks')
            x_overall = [x_overall mouse_costLicks];
        elseif isequal(xType,'Licks')
%             x_overall = [x_overall mouse_licks];
            x_overall = [x_overall mouse_rewardLicks mouse_costLicks];
        end
        ...
        elseif isequal(yType,'Trace Sum')
%             y_overall = [y_overall mouse_sum];
            y_overall = [y_overall mouse_reward mouse_cost];
    ...
    supertitle({[intendedStriosomality ' ' health  ' ' learned_str ' ' age ' N=' num2str(numMice)]...
        ,[num2str(learningPeriod) ' of Learning'],['Bin Size: ' num2str(bin_size), eng_str],'',''})
