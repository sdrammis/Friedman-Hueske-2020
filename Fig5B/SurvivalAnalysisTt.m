function SurvivalAnalysisTt(twdb,miceType)
%tt survival analysis with option to add thresholds
...
for i = 1:length(IDs)
    ...
    Learned = cell2mat(twdb_lookup(miceType, 'learnedFirstTask', 'key', 'ID', ID));
    if Learned == -1
...
TrialDataLearned = get_mouse_trials_2(twdb, LearnedID, true, true);
TrialDataNotLearned = get_mouse_trials_2(twdb, NotLearnedID, true, true);
TrialData = get_mouse_trials_2(twdb, IDs, true, true);
...
for i = 1:length(IDs)
    ...
    learned = first(twdb_lookup(twdb, 'learnedFirstTask', 'key', 'mouseID', IDs(i)));
   % if learned == -1 && size(TrialData{1, i}, 1) < meanLearned + STDLearned
...
titleStr = 'Two Tone Survival Analysis';
...
tTYng_HD_vs_WT = dg_chi2test3(n1)
tTMid_HD_vs_WT = dg_chi2test3(n2)
tTOld_HD_vs_WT = dg_chi2test3(n3)
tTWT_yng_vs_mid = dg_chi2test3(n4)
tTWT_yng_vs_old = dg_chi2test3(n5)
tTWT_mid_vs_old = dg_chi2test3(n6)
tTHD_yng_vs_mid = dg_chi2test3(n7)
tTHD_yng_vs_old = dg_chi2test3(n8)
tTHD_mid_vs_old = dg_chi2test3(n9)

tTmean_learned = meanLearned
tTmean_not_learned = meanNotLearned
tTSTD_learned = STDLearned
tTSTD_not_learned = STDNotLearned
tTfiltered_mice = length(filtered)
