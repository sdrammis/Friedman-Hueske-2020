function [mean_rz,mean_cz,RTS,CTS,DP] = calcZRespLicks3_QZ(mouseIDs,...
    miceTrials,miceFTrials,rTones,cTones,binSize,lowLick,highLick)
mean_rz = [];
mean_cz = [];
RTS = [];
CTS = [];
...
%     disp(['------' num2str(i) ': ' mouseIDs{i} '------'])
    ...
    rangeIdxs = find(respLicks >= lowLick & respLicks <= highLick);
    mouseTrials = mouseTrials(rangeIdxs,:);
    mouseFTrials = mouseFTrials(rangeIdxs,:);
    respLicks = respLicks(rangeIdxs);
    ...
    for j = 2:length(binCuts)
        ...
        if isnan(rts) || isnan(cts) || isnan(dp)
            continue;
        end
